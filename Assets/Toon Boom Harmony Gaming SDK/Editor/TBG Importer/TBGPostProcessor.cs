...
namespace ToonBoom.TBGImporter
{
    ...
    {
        public static IEnumerable<AnimationClip> GetSavedClips(string path) =>
            AssetDatabase.LoadAllAssetsAtPath(path)
                .Select(asset => asset as AnimationClip)
                .Where(asset => asset != null);
        public static void ApplyAnimationClipsToAnimatorController(TBGImporter tbgImporter, IEnumerable<AnimationClip> savedClips)
        {
        ...
        static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
        {
            foreach (var imported in importedAssets)
            {
                ...
                if (assetImporter is not TBGImporter)
                    continue;
                var tbgImporter = (TBGImporter)assetImporter;
                ApplyAnimationClipsToAnimatorController(tbgImporter, GetSavedClips(assetImporter.assetPath));
